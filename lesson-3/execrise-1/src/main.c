...
#include <zephyr/bluetooth/conn.h>  // For managing BLE connections.
...
void on_connected(struct bt_conn *conn, uint8_t err) {
  ...
  dk_set_led(CONNECTION_STATUS_LED, LED_ON);
}
...
void on_disconnected(struct bt_conn *conn, uint8_t reason) {
  ...
  dk_set_led(CONNECTION_STATUS_LED, LED_OFF);
}

/* Declare connection_callback structure. */
